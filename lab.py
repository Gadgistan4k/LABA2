...
with open('anime.csv', newline='', encoding='utf-8') as csvfile:
    reader = list(csv.DictReader(csvfile, delimiter=',')) # Открываю файл
    for row in reader:
        all_anime.append(row['Name']) # Создаю файл с перечнем всего аниме

print('Пройдите опрос (если пункт не важен, нажмите Enter)')
answer = [] # Создаю масси в с ответами
for i in questions:
    n = input(i)
    answer.append(n)
answer = dict(zip(answers, answer)) # Создаю массив ответов с ключами
...
# По каждой фарактеристики создаю массив с названиеми аниме которые не подходят по характеристикам

for bad in (anime_stu, anime_seas, anime_end, anime_start, anime_epi, anime_type, anime_cont, anime_tag, anime_rai):
    for ani in bad:
        if ani in all_anime:
            all_anime.remove(ani) # Убираю из общего пула не подходящие названия сформированные ранее

if len(all_anime) != 0:
    with open("youranime.txt", 'w', encoding='utf-8') as res:
        for i in all_anime:
            res.write(f'{i}\n') # Добавляю в файл подходящие аниме
